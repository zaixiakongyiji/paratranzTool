import{S as o}from"./storage-D0vRnQMA.js";import{s as a}from"./toast-BNFNXAUb.js";function p(t){c(t),i()}function c(t){t.innerHTML=`
  ...
  `,document.getElementById("btn-add-local-term").addEventListener("click",()=>{const n=document.getElementById("new-term-key").value.trim(),e=document.getElementById("new-term-val").value.trim();!n||!e||(o.addLocalTerm(n,e),document.getElementById("new-term-key").value="",document.getElementById("new-term-val").value="",i(),a("已添加到本地知识库","success"))}),document.getElementById("btn-export-json").addEventListener("click",()=>{const n=o.getLocalGlossary(),e=new Blob([JSON.stringify(n,null,2)],{type:"application/json"}),r=URL.createObjectURL(e),l=document.createElement("a");l.href=r,l.download=`my_glossary_${new Date().toISOString().slice(0,10)}.json`,l.click(),a("已开始导出","success")}),document.getElementById("btn-import-json").addEventListener("click",()=>{document.getElementById("file-import-json").click()}),document.getElementById("file-import-json").addEventListener("change",n=>{const e=n.target.files[0];if(!e)return;const r=new FileReader;r.onload=l=>{try{const s=JSON.parse(l.target.result);Array.isArray(s)?(o.saveLocalGlossary(s),i(),a("导入成功","success")):a("JSON 格式不正确，应为数组","error")}catch{a("JSON 解析失败","error")}},r.readAsText(e)})}function i(){const t=document.getElementById("local-term-list"),n=o.getLocalGlossary();if(n.length===0){t.innerHTML='<div style="text-align: center; color: var(--text-secondary); padding: 2rem;">本地知识库目前为空</div>';return}t.innerHTML=n.map((e,r)=>`
  ...
  `).join(""),t.querySelectorAll(".btn-remove-term").forEach(e=>{e.addEventListener("click",()=>{o.removeLocalTerm(e.dataset.term),i()})})}function d(t){return(t||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}export{p as render};
