...
import{n as R,_ as j}from"./index-CR5PaB1c.js";import{paraTranzApi as q}from"./paratranz-CmxxXTVm.js";import{AIClient as D}from"./ai-OpbjDDFA.js";import{Storage as L}from"./storage-gCDr8-fQ.js";import{showToast as b}from"./toast-_A8NGhfi.js";async function K(m,d){const f=d.get("projectId"),a=d.get("fileId");if(!f||!a){m.innerHTML='<div class="glass-panel text-center">缺少 projectId 或 fileId</div>';return}const E=d.get("stage")||"0";m.innerHTML='<div style="text-align:center; padding: 2rem;">加载词条数据中...</div>';try{let[w,o]=await Promise.all([q.getTerms(f).catch(()=>[]),q.getStrings(f,a,E==="all"?null:E)]);const v=L.getLocalGlossary(),h=new Map;w.forEach(x=>h.set(x.term,x.translation)),v.forEach(x=>h.set(x.term,x.translation));const M=Array.from(h.entries()).map(([x,S])=>({term:x,translation:S}));o=o||[],_(m,f,a,o,M,E)}catch(w){m.innerHTML=`<div class="glass-panel" style="color: var(--danger-color)">初始化失败 ${w.message}</div>`}}function _(m,d,f,a,E,w){let o=-1,v="none",h=[];function M(){m.innerHTML=`
