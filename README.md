...
- **RAG 翻译增强**：将项目全部历史译文向量化后索引，每翻一条新词条时检索最相似的历史翻译，同时智能拼接该词条的前后句语境注入 AI prompt 作为参考，解决游戏汉化中风格一致性的核心痛点。支持严谨的增量同步校验机制。
- **多模型可插拔**：翻译主模型、向量化模型、过滤模型、重排序模型各自独立配置，全部兼容 OpenAI 和 Gemini 双协议（支持 Gemini 批量 Embedding 优化）。
